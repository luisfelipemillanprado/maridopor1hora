...
export default function CompanyInformation(props) {
    ...
    return (<div className={clsx(`horizontal h-auto w-full justify-center gap-x-5`)}>
      {companyInformation.links.map((item /*, index */) => (<InformationLink key={item.key} {...{ href: item.href, text: item.text }}/>))}
    </div>);
}
