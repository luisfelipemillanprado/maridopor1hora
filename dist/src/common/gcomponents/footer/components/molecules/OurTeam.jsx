...
export default function OurTeam(props) {
    ...
    return (<div className={clsx(`vertical h-auto w-full gap-y-4`)}>
      <div className={clsx(`vertical w-full items-center gap-y-2`)}>
        <span className={clsx(`text-default-50 text-[1.1875rem]`, `2xl:text-xl`, `3xl:text-[1.1875rem]`)}>
          {text1}
        </span>
      ...
      <div className={clsx(`horizontal h-auto w-full justify-center`)}>
        <AvatarGroup isBordered max={6} total={10} size={`sm`}>
          {ourTeam.members.map((item /*, index */) => (<TeamMember key={item.key} {...{ src: item.imageUrl }}/>))}
        </AvatarGroup>
      </div>
    </div>);
}
