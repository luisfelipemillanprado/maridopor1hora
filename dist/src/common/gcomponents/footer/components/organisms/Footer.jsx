...
export default function Footer(props) {
    const { companyInformation, ourTeam, fourthTitle, socialNetworks, secondaryDescription, contactUs, termsConditions, otherlinks, } = props;
    return (<div className={clsx(`vertical bg-content5 h-auto w-full !justify-start gap-y-8 rounded-t-3xl pt-7`)}>
      ...
      <div className={clsx(`horizontal h-auto w-full justify-between`, `2xl:px-6`)}>
        <section aria-label={`company-links-information`} className={clsx(`hidden h-auto w-auto`, `3xl:flex 3xl:flex-row`)}>
          <OtherLinks {...{ titles: otherlinks.titles, links: otherlinks.links }}/>
        </section>
        <div className={clsx(`vertical h-auto w-full !justify-start gap-y-9`, `3xl:w-auto 3xl:h-[20.8125rem]`)}>
          <section aria-label={`company-team`} className={clsx(`horizontal h-auto w-full justify-center px-4`, `3xl:w-auto`)}>
            <OurTeam {...{ ourTeam, text1: fourthTitle[0].text1, text2: fourthTitle[0].text2 }}/>
          </section>
          <section aria-label={`company-social-networks`} className={clsx(`horizontal h-auto w-full justify-center px-4`, `3xl:w-auto`)}>
            <SocialNetworks {...{ socialNetworks, text1: fourthTitle[1].text1 }}/>
          </section>
        </div>
      </div>
      <section aria-label={`company-secondary-description`} className={clsx(`hidden h-auto w-full justify-center px-4`)}>
        <SecondaryDescription {...{ text: secondaryDescription[0] }}/>
      </section>
      <section aria-label={`company-contact-us`} className={clsx(`horizontal h-auto w-full justify-center px-4`)}>
        <ContactUs {...{ contactUs }}/>
      </section>
      <section aria-label={`company-terms-conditions`} className={clsx(`horizontal border-default-600 h-auto w-full justify-center border-t-1 p-4`)}>
