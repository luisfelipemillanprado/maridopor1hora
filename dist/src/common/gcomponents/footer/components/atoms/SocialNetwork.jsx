...
export default function SocialNetwork(props) {
    ...
    const iconSwitch = (icon) => {
        switch (icon) {
            ...
            case `instagram`:
                return {
                    icon: (<InstagramIcon className={clsx(`h-[1.6875rem] w-[1.6875rem]`, `2xl:h-9 2xl:w-9`)}/>),
                };
            case `whatsapp`:
                return {
    ...
    return (<Button isIconOnly isExternal className={clsx(`bg-content3`, `2xl:h-9 2xl:w-9`, `3xl:h-8 3xl:w-8`)} aria-label={`social-network-${icon}`} as={Link} href={href} color={`default`} variant={`faded`} radius={`full`} size={`sm`}>
      {(_a = iconSwitch(icon)) === null || _a === void 0 ? void 0 : _a.icon}
    </Button>);
}
