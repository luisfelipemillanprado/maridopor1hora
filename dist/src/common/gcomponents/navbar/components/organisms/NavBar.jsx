...
export default function NavBar(props) {
    const { navbar } = props;
    return (<Navbar className={clsx(`h-[3.75rem] bg-inherit pt-1.5 backdrop-blur-none [&>header]:px-4`, `3xl:[&>header]:px-8`)}>
      <NavbarContent className={clsx(``)} justify={`start`}>
        ...
        <NavbarContent className={clsx(`hidden gap-x-8 sm:flex`)}>
          {navbar.links.map((item /*, index*/) => (<NavLink key={item.key} {...{ text: item.text, href: item.href }}/>))}
        </NavbarContent>
      </NavbarContent>
