import { Avatar, Badge, Dropdown, DropdownTrigger, DropdownMenu, DropdownSection, DropdownItem, Button, } from '@heroui/react';
...
export default function Avatars(props) {
    ...
    return (<Dropdown>
      <DropdownTrigger>
        <Button className={clsx(`vertical h-11 w-11 min-w-11 items-center p-2`, `3xl:h-12 3xl:w-12`)} variant={`light`}>
          <Badge color={`success`} content={content} shape={`circle`} variant={`solid`} placement={`bottom-right`} size={`sm`} showOutline={false}>
            <Avatar className={clsx(`h-[2.1875rem] w-[2.1875rem]`, `3xl:h-[2.3125rem] 3xl:w-[2.3125rem]`)} isBordered size={`sm`} color={`warning`} src={imageUrl}/>
          </Badge>
        </Button>
      </DropdownTrigger>
      <DropdownMenu aria-label={`dropdown-avatar-menu`} variant={`faded`}>
        <DropdownSection showDivider title={`Actions`}>
          {items.map((item /* , index */) => (<DropdownItem key={item.key} description={item.description} shortcut={item.shortcut} startContent={<MenuIcons {...{ icon: item.key }}/>}>
              {item.title}
            </DropdownItem>))}
        </DropdownSection>
      </DropdownMenu>
