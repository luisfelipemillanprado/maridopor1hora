...
export default function FQAs(props) {
    ...
    return (<Accordion disableAnimation className={clsx(`gap-0 gap-y-3 px-0`)} variant={`splitted`}>
      {questions.map((item, index) => (<AccordionItem key={item.key} className={clsx(`border-content3 bg-content2 border-1 pb-3.5`, `[&>h2>button]:py-0 [&>h2>button]:pt-3.5 [&>h2>button]:pb-0`)} aria-label={`FQAs ${index + 1}`} startContent={<FQAIcon {...{ icon: item.icon }}/>} title={<h3 className={clsx(`text-default-900 text-large font-bold`, `3xl:text-[1.1875rem] 2xl:text-[1.1875rem]`)}>
              {item.question}
            </h3>}>
