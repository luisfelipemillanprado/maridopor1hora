...
export default function TrustBadge(props) {
    ...
    return (<Card className={clsx(`bg-content3 border-content4 h-auto w-full border-1 px-1`)} shadow={`md`}>
      <CardHeader className={clsx(`horizontal pb-2`)}>
        <div className={clsx(``)}></div>
        <div className={clsx(`vertical gap-y-2.5`)}>
          <h3 className={clsx(`text-default-900 text-[1.3125rem] font-bold text-shadow-md`, `3xl:text-[1.375rem] 2xl:text-[1.3125rem]`)}>
            {trustBadge.slogan}
          </h3>
      ...
      <CardBody className={clsx(`horizontal justify-between`)}>
        <div className={clsx(`horizontal h-auto w-auto`)}>
          <Button className={clsx(`bg-content1`)} startContent={<BoltIcon className={clsx(`fill-warning-500 h-4 w-4`, `3xl:w-5 3xl:h-5 2xl:h-[1.0625rem] 2xl:w-[1.0625rem]`)} aria-hidden={true}/>} variant={`flat`} radius={`full`} size={`sm`} aria-label={trustBadge.buttonText} title={trustBadge.buttonText}>
            <span className={clsx(`text-small text-default-700 h-auto w-auto`, `3xl:text-medium 2xl:text-[0.9375rem]`)}>
