...
export default function WhyChooseUs(props) {
    ...
    return (<Swiper slidesPerView={`auto`} loop={true} autoplay={{
            delay: 7000,
            disableOnInteraction: false,
        }} modules={[Autoplay]} className={clsx(`h-auto w-full !py-2.5 !pl-1.5`)} aria-label={`Highlighted benefits`}>
      {benefits.map((item) => (<SwiperSlide key={item.key} className={clsx(`mr-5 !h-auto !w-auto`, `3xl:mr-6`)}>
          <Benefit {...{
            title: item.title,
