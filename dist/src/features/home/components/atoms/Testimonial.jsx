...
export default function Testimonial(props) {
    ...
    return (<div className={clsx(`vertical h-full w-full items-center justify-start gap-y-1 p-4`)}>
      <div className={clsx(`vertical mb-1 h-auto w-full items-center gap-y-2`)}>
        <div className={clsx(`horizontal border-warning-500 border-1.5 h-auto w-auto justify-center rounded-full border-dashed p-1`)}>
          <Image width={57} height={57} className={clsx(`h-[3.5625rem] w-[3.5625rem] rounded-full shadow-md`, `2xl:h-[3.75rem] 2xl:w-[3.75rem]`)} src={imageUrl} blurDataURL={blurEffects.blurUrl} placeholder={`blur`} alt={`Photo by ${name}, satisfied customer`} itemProp={`image`}/>
        </div>
        <div className={clsx(`horizontal h-auto w-auto justify-between gap-1`)}>
          {score.map((item) => {
            return (<span key={item.key} className={clsx(`horizontal h-auto w-auto justify-center`)}>
                <StarIcon className={clsx(`fill-warning-500 h-[1.125rem] w-[1.125rem]`, `2xl:h-[1.1875rem] 2xl:w-[1.1875rem]`)} aria-hidden="true"/>
              </span>);
        })}
        </div>
      </div>
