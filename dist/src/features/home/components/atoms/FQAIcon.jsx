 ...
 */
export var FQAIconType;
(function (FQAIconType) {
    FQAIconType["QUESTION"] = "question";
    FQAIconType["MORE"] = "more";
})(FQAIconType || (FQAIconType = {}));
/**
...
const iconSwitch = (icon) => {
    switch (icon) {
        case FQAIconType.QUESTION:
            return (<InformationCircleIcon aria-label={`information`} className={clsx(`fill-warning h-[1.625rem] w-5`)}/>);
        case FQAIconType.MORE:
            return (<EllipsisVerticalIcon aria-label={`ellipsis`} className={clsx(`fill-warning h-[1.625rem] w-5`)}/>);
        default:
 ...
 */
export default function FQAIcon(props) {
    const { icon } = props;
