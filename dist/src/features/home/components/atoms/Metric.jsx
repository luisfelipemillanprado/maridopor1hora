...
(function (MetricIcon) {
    MetricIcon["FRECUENTES"] = "frequent";
    MetricIcon["SATISFECHOS"] = "satisfied";
    MetricIcon["CALIFICADOS"] = "qualified";
...
export default function Metric(props) {
    const { metric } = props;
    const changeElement = metric.change ? (<span className={clsx(`text-warning text-large font-semibold`, `3xl:text-xl 2xl:text-[1.1875rem]`)}>
      k
    </span>) : (<PlusIcon className={clsx(`h-[1.1875rem] w-[1.1875rem]`, `3xl:h-[1.375rem] 3xl:w-[1.375rem] 2xl:h-5 2xl:w-5`)}/>);
    return (<Card isPressable className={clsx(`outline-warning-200 bg-content7 w-[6.125rem] gap-y-0.5 outline-1 outline-solid`, `3xl:w-auto ${metric.change ? 'shadow-md' : '3xl:shadow-medium'} 2xl:w-[6.4375rem]`)} shadow={`sm`}>
      <CardHeader className={clsx(`horizontal justify-center gap-x-0.5 px-1 pt-2 pb-0`, `3xl:px-4 3xl:pt-4 3xl:pb-1.5 3xl:justify-between`)}>
        ...
        <div className={clsx(`horizontal h-auto w-auto`, `3xl:pr-4.5`)}>
          <span className={clsx(`text-warning text-[1.1875rem] font-semibold`, `3xl:text-xl 2xl:text-[1.1875rem]`)}>
            {metric.amount}
          </span>
          <span className={clsx(`text-warning text-[1.0625rem]`)}>{changeElement}</span>
        </div>
      </CardHeader>
      <CardBody className={clsx(`vertical !scrollbar-hide px-1.5 pt-0 pb-2.5`, `3xl:px-5 3xl:pb-5 3xl:gap-y-1.5`)}>
        <span className={clsx(`text-center text-[0.9375rem]`, `2xl:text-medium 3xl:text-large 3xl:text-start xl:text-[0.9375rem]`)}>
          {metric.text}
        </span>
