...
export default function Service(props) {
    ...
    return (<Card isPressable className={clsx(`bg-content3 border-content4 col-span-6 gap-y-2 border-1 p-1`, `3xl:p-1.5 4xl:col-span-4`)} shadow={`md`}>
      <CardHeader className={clsx(`justify-between pb-2`)}>
        <Avatar isBordered className={clsx(`3xl:h-[2.625rem] 3xl:w-[2.625rem]`)} color={`warning`} radius={`full`} size={`md`} src={imageUrl} alt={`Image service ${name}`} itemProp={`image`}/>
      </CardHeader>
      ...
      <CardFooter className={clsx(`horizontal items-start pt-0`)}>
        <p className={clsx(`text-default-800 text-medium truncate`, `3xl:overflow-visible 3xl:whitespace-normal 3xl:text-start 2xl:text-[1.0625rem]`)}>
          {description}
        </p>
      </CardFooter>
    </Card>);
