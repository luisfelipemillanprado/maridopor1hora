...
export default function JoinTeamButton(props) {
    ...
    return (<div className={clsx(`horizontal h-auto w-auto`)}>
      <Button isExternal aria-label={`join-team`} className={clsx(`bg-warning`)} endContent={<UserGroupIcon className={clsx(`fill-content5 h-6 w-6`)}/>} as={Link} href={href} variant={`shadow`} size={`md`} radius={`full`}>
